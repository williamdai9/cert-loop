...
type ResearchRow = { id: string; external_id: string; title: string; abstract?: string; source_url: string };
...
async function curateWithGateway(request: Request) {
  ...
  const rows = await sourceResponse.json() as ResearchRow[];
  ...
  const aiResponse = await fetch("https://ai-gateway.vercel.sh/v1/responses", { method: "POST", headers: { Authorization: `Bearer ${gatewayKey}`, "Content-Type": "application/json" }, body: JSON.stringify({
    ...
    instructions: `Curate a daily CSCS evidence watch. The English fifth-edition textbook and official NSCA DCO remain exam truth. From each abstract, create a cautious English digest, Chinese support note, CSCS chapter mapping, relevance 0-1, and at most one original three-option application question. Never overstate causality or turn a single study into an exam rule. Return JSON only: {"items":[{"id":"uuid","summary_en":"","summary_zh":"","chapter_numbers":[1],"relevance":0.5,"question":null|{"domain_id":"","prompt":"","options":["","",""],"answer_index":0,"explanation":""}}]}.`,
  ...
  for (const item of digest.items || []) {
    ...
    curated += 1;
    if (item.question) await fetch(`${base}/rest/v1/content_review_queue`, { method: "POST", headers: { ...headers, Prefer: "return=minimal" }, body: JSON.stringify({ certification_id: "nsca-cscs-5", item_type: "question", confidence: item.relevance, payload: { origin: "daily-research", research_item_id: item.id, status_note: "AI draft; human review required before question-bank publication", ...item.question } }) });
  }
  return { curated };
}
