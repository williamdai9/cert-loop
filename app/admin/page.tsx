...
type AuditChapter = {
  ...
  knowledgePoints: number; recallChecks: number; atlasModels: number; noteFigures: number;
...
type AdminPayload = {
  ...
  audit: {
    ...
    totals: { chapters: number; sections: number; explanationParagraphs: number; knowledgePoints: number; questions: number; atlasModels: number; noteFigures: number; mindMaps: number; figureAuditsComplete: number };
  };
  cloud: {
...
function Overview({ data }: { data: AdminPayload }) {
  ...
  const checks = [
    ...
    { ok:data.audit.totals.figureAuditsComplete === 26, warn:true, label:"Whole-book figure audit", note:`${data.audit.totals.figureAuditsComplete}/26 chapters complete; Chapters 2–26 remain an explicit backlog` },
  ...
  return <>
    ...
    <section className={styles.metrics}>{[
      [totals.chapters,"chapters"],[totals.sections,"deep dives"],[totals.knowledgePoints,"knowledge points"],[totals.questions,"questions"],[totals.atlasModels,"atlas models"],[totals.noteFigures,"note figures"],[totals.mindMaps,"mind maps"],[data.cloud.research.length,"research items"],
